...
/// Das globale Array, das in der cosmos_ahci.cpp liegt
_172 DriveInfo drives[8];
/// ==========================================
/// DAS TRAMPOLIN (Absoluter Weitsprung)
/// ==========================================
_81 _50 (*trampoline_jump_t)();
_50 jump_to_tree(_89 ziel_adresse) {
    trampoline_jump_t absoluter_sprung = (trampoline_jump_t)ziel_adresse;
    absoluter_sprung(); /// Abflug!
}
/// ==========================================
...
_202 Star { _43 x, y, z; };
/// ==========================================
/// BARE METAL FIX: Das Spiegel-System (Geräte-Manager)
/// ==========================================
_202 MirrorDevice { 
    _184 bus, dev, func; 
    _182 vendor, device; 
    _89 bar0; 
    _89 reflection; /// Das "Ich bin Du!"
    _30 name[32]; 
};
...
/// FS & NAV
_30 hw_net[48] = "SCANNING..."; _30 hw_gpu[48] = "VGA/VESA"; _30 hw_usb[48] = "NO USB"; _30 hw_storage[48] = "SCANNING...";
_172 _30 hw_disk[48]; 
...
/// Globale Test-Variablen (oben bei den anderen definieren)
...
/// BARE METAL FIX: Kein "_172"! Hier werden sie ECHT in den RAM gebaut.
OracleEntry oracle_db[64];
_43 oracle_entry_count = 0;
/// =========================================================
/// BARE METAL FIX: Das komplette AHCI Dateisystem-Update
/// =========================================================
_50 ahci_save_file_complete(HBA_PORT* hdd, _71 _30* filename, _184* data_buf) {
    ...
    _15(slot EQ 255) _96; /// Ordner ist voll (mehr als 8 Dateien)!
    ...
    /// ...und ZURÜCKSCHREIBEN! (Das hat vorher gefehlt!)
    ahci_write(hdd, 1002, dir_buf); 
}
/// --- BARE METAL FIX: PCI BRÜCKEN ---
_172 _89 inl(_182 p);
...
_50 pci_find_uhci() {
        ...
        _39(_43 slot = 0; slot < 32; slot++) {
            _39(_43 func = 0; func < 8; func++) {
                ...
                _15(class_code EQ 0x0C AND subclass EQ 0x03) {
                    found_any_usb = 1;
                    /// Schauen wir mal, was die Realität sagt!
                    _15(prog_if EQ 0x00) {
    ...
    /// Wenn gar kein USB-Chip gefunden wurde
    _15(!found_any_usb) str_cpy(hw_usb, "NO USB HARDWARE ON PCI");
}
/// ==========================================
/// BARE METAL FIX: USB STICK REGISTRIERUNG
/// ==========================================
_182 inw(_182 p);
/// ==========================================
/// BARE METAL FIX: ECHTE UHCI PORT-MESSUNG
/// ==========================================
_50 usb_mount_drive() {
    ...
    /// Für das SYS Fenster schon mal den Standard-Text setzen
    _15(usb_found > 0) {
...
/// BARE METAL FIX: Dem Compiler die Grafik-Befehle vorstellen
_50 TextC(_43 cp, _43 y, _71 _30* s, _89 col, _44 bold);
_50 Swap();
_50 kernel_panic(_71 _30* error_msg) {
    __asm__ _192("cli"); /// Interrupts stoppen
    _39(_43 i=0; i < 800*600; i++) bb[i] = 0x0000FF; /// Blau färben
    TextC(400, 200, "COSMOS OS - CRITICAL ERROR", 0xFFFFFF, _128);
    TextC(400, 250, error_msg, 0xFFFFFF, _86);
    TextC(400, 400, "Noob! SYSTEM STOPPED TO PROTECT HARDWARE. You NOOB", 0xAAAAAA, _86);
    Swap(); 
...
_50 boot_logo() {
    ...
    TextC(c,y,"#####################################",g,_128);
    TextC(c,y+20,"         SCHNEIDER SYSTEM	        ",0xFFFFFF,_128);
...
_50 play_earthquake(_43 duration) { shake_timer = duration; _114(shake_timer > 0) { play_freq(40 + (random() % 60)); Swap();
_39(_192 _43 d=0; d<40000; d++); } play_freq(0); }
...
_50 handle_global_keys() {
    ...
    /// ==========================================
    /// DIE RETTUNG VOR DEM GEISTER-SHIFT
    /// Wenn ein Fenster die Taste zum Navigieren benutzt hat, 
    /// löschen wir sie aus dem System. Der Text-Editor sieht sie nie!
    /// ==========================================
    _15(key_was_eaten) {
        key_new = _86; 
    }
...
/// ==========================================
/// UI BUTTON HANDLER: KERNEL BOOTEN
/// ==========================================
/// 'selected_port' ist die Port-Nummer der Festplatte/CD, 
/// die du in deinem UI (im Disk Manager) gerade markiert hast.
/// 2. Der Button-Klick (Der Zünder)
_50 on_boot_button_click(_43 port_no) {
    /// A. Lade deine ausführbare Datei (z.B. TREE.BIN oder KERNEL.BIN) in den RAM.
    /// Ich gehe davon aus, du nutzt hier deine ISO-Ladefunktion und 16MB (0x1000000)
    iso9660_load_file(port_no, "TREE    BIN");
    /// B. Mache den Bildschirm komplett schwarz, damit dein neues OS/Tree 
    /// eine völlig saubere Leinwand für seine eigenen Grafiken hat.
    _39(_43 p=0; p<1024*768; p++) Put(p%1024, p/1024, 0x000000);
    Swap();
    /// C. DER ABSPRUNG! Keine Rückkehr mehr in dieses UI.
    jump_to_tree(0x1000000); 
}
/// --- DATEI-LADE-SPRUNG (Jump to Loaded File) ---
_50 load_and_run_app(_43 port_no, _89 start_lba, _43 sector_count) {
    _89 load_addr = 0x1000000;
    /// 1. Datei von SSD in den RAM schaufeln
    _15(ahci_read_sectors(port_no, start_lba, sector_count, load_addr)) {
        str_cpy(cmd_status, "APP LOADED. JUMPING...");
        /// 2. Der Trampolin-Sprung
        jump_to_tree(load_addr); 
    } _41 {
        str_cpy(cmd_status, "DISK READ ERROR!");
    }
}
/// ==========================================
...
_89 force_find_ahci() {
    _39(_43 bus = 0; bus < 256; bus++) {
        _39(_43 slot = 0; slot < 32; slot++) {
            /// BARE METAL FIX: Die 8 Funktionen eines PCIe-Geräts MÜSSEN gescannt werden!
            _39(_43 func = 0; func < 8; func++) {
                /// Hier wurde vorher immer hart '0' statt 'func' übergeben!
                ...
                _15(class_sub EQ 0x0106) {
                    _89 bar5 = read_pci_reg(bus, slot, func, 0x24);
...
///  KERNEL MAIN
/// ==========================================
_89* nvidia_vram_addr = 0;
_172 _50 ahci_mount_drive();
_172 _50 ahci_read_mbr();
_172 _50 fs_init();
_172 "C" _50 kernel_main(_89 magic, _89* mbi) {
    /// 1. VESA Framebuffer von GRUB übernehmen
    _15 (magic == 0x2BADB002) {
        _15 (mbi[0] & (1 << 12)) {
            fb = (_89*)(uintptr_t)mbi[22];
            pitch = mbi[24]; screen_w = mbi[25]; screen_h = mbi[26];
        } _41 {
            fb = (_89*)0xFD000000; pitch = 800 * 4; screen_w = 800; screen_h = 600;
        }
    }
    get_cpu_brand();
    startup_melody();
	...
	fs_init();
    /// ==========================================
    /// 2. HARDWARE ZWINGEN & MOUNTEN
    /// ==========================================
    global_ahci_abar = force_find_ahci();
    _15(global_ahci_abar NEQ 0) {
        ahci_init(global_ahci_abar);
    ...
    _114(1) {
		...
		_15(active_mode EQ 0) { 
            ...
            _39(_43 i=0;i<200;i++){ stars[i].z -= 2; _15(stars[i].z <= 0) { stars[i].z = 1000; 
			stars[i].x = (random()%799)-399; stars[i].y = (random()%599)-299; } 
			Put(v_cx + (stars[i].x * 256) / stars[i].z, v_cy + (stars[i].y * 256) / stars[i].z, 0x444444); }
            _15(galaxy_open AND galaxy_expansion < 320) galaxy_expansion += 8; 
			_15(!galaxy_open AND galaxy_expansion > 0) galaxy_expansion -= 10;
            DrawCentrifugalVortex(v_cx, v_cy, galaxy_expansion); 
			DrawRoundWindow(v_cx, v_cy, 50, 0xFFD700); TextC(v_cx, v_cy-15, "COSMOS", 0x000000, _128);
            ...
            _39(_43 i=0; i<13; i++) {
				...
				_15(is_modal_blocked AND k NEQ 2) win_bg = 0x888888;
                ...
                _15(win->id EQ 3) { 
                    ...
                    _15(input_cooldown EQ 0 AND mouse_just_pressed AND !blocked AND is_over_rect(mouse_x, mouse_y, wx+5, btn_y, 140, 20)) { 
					sys_lang = !sys_lang; input_cooldown = 10; }
                    Text(wx+10, btn_y+4, lang_lbl, 0x000000, _128);
                    _15(input_cooldown EQ 0 AND mouse_just_pressed AND !blocked AND is_over_rect(mouse_x, mouse_y, wx+5, btn_y+30, 200, 20)) { 
                    ...
                    DrawRoundedRect(wx+20, wy+375, ww-40, 25, 4, 0x00FF00); 
                    TextC(mid, wy+382, "OPEN HARDWARE ORACLE", 0x000000, _128);
                    _15(mouse_just_pressed AND !blocked AND is_over_rect(mouse_x, mouse_y, wx+20, wy+375, ww-40, 25)) {
                    ...
                    /// --- BARE METAL FIX: Storage klickbar ---
                    ...
                    Text(wx+30, wy+280, l_disk, 0x000000, _86); /// Schwarz, da anklickbar
                    Text(wx+190, wy+280, hw_disk, 0x222222, _86);
                    /// --- NETZWERKKARTEN ---
					_15(mouse_just_pressed AND !blocked AND is_over_rect(mouse_x, mouse_y, wx+30, wy+300, 250, 20)) {
                        input_cooldown = 10;
                    }
                    Text(wx+30, wy+305, l_net, 0x000000, _86);  /// Schwarz, da anklickbar
                    Text(wx+190, wy+305, hw_net, 0x222222, _86);
                    /// --- BARE METAL FIX: USB INTERAKTIV IM SYS-FENSTER ---
                    _15(mouse_just_pressed AND !blocked AND is_over_rect(mouse_x, mouse_y, wx+30, wy+350, 250, 20)) { 
                        _15(drive_count > 0) {
                            _72 _43 view_usb_idx = 0; /// static Variable (merkt sich den Index)
                            _43 found = 0;
                            /// Wir suchen maximal drive_count-mal, absolut sicherer Schutz vor Aufhängern!
                            _39(_43 i=0; i<drive_count; i++) {
                                view_usb_idx++;
                                _15(view_usb_idx >= drive_count) view_usb_idx = 0;
                                _15(drives[view_usb_idx].type EQ 3) {
                                    str_cpy(hw_usb, drives[view_usb_idx].model);
                                    found = 1;
                                    _37;
                                }
                            }
                            _15(!found) str_cpy(hw_usb, "NO USB FOUND");
                        }
                        input_cooldown = 10;
                    }
                    /// --- GPU & USB RENDERING ---
                    Text(wx+30, wy+330, l_gpu, 0x555555, _86);  
                    Text(wx+190, wy+330, hw_gpu, 0x222222, _86);
                    /// USB Text in Schwarz (0x000000) zeichnen, damit der User sieht, dass es klickbar ist!
                    Text(wx+30, wy+355, l_usb, 0x000000, _86);  
                    Text(wx+190, wy+355, hw_usb, 0x222222, _86);
				...
				/// =========================================================
                /// APP: GERÄTE-MANAGER & SPIEGEL-SYSTEM (ID 10)
                /// =========================================================
                _15(win->id EQ 10) {
                    TextC(wx+ww/2, wy+40, "SPIEGEL-SCAN: 'DU BIST ICH, ICH BIN DU'", 0x00FF00, _128);
                    DrawRoundedRect(wx+20, wy+60, 150, 25, 3, 0x333333);
                    TextC(wx+95, wy+68, "INITIATE SCAN", 0xFFFFFF, _128);
                    _15(input_cooldown EQ 0 AND mouse_just_pressed AND !blocked AND is_over_rect(mouse_x, mouse_y, wx+20, wy+60, 150, 25)) {
                        mirror_count = 0;
                        /// Das Mainboard abtasten
                        _39(_43 b=0; b<256; b++) {
                            _39(_184 d=0; d<32; d++) {
                                _39(_184 f=0; f<8; f++) {
                                    _89 id = pci_read(b, d, f, 0);
                                    _182 ven = id & 0xFFFF;
                                    _15(ven NEQ 0xFFFF AND mirror_count < 20) {
                                        ...
                                        /// ----------------------------------------------------
                                        /// DER SPIEGEL-TEST (Wer bist du!?)
                                        /// ----------------------------------------------------
                                        _89 orig_bar = pci_read(b, d, f, 0x24); /// Lese BAR5 (für AHCI)
                                        pci_write(b, d, f, 0x24, 0xFFFFFFFF);   /// Anschreien!
                                        _89 reflection = pci_read(b, d, f, 0x24); /// Echo abfangen
                                        pci_write(b, d, f, 0x24, orig_bar);     /// Frieden wiederherstellen
                                        mirror_list[mirror_count].bar0 = orig_bar;
                                        mirror_list[mirror_count].reflection = reflection;
                                        /// Geräte-Klasse identifizieren
                                        _89 class_rev = pci_read(b, d, f, 0x08);
                                        _184 cls = (class_rev >> 24) & 0xFF;
                                        _184 sub = (class_rev >> 16) & 0xFF;
                                        ...
                                        _41 _15(cls EQ 0x0C AND sub EQ 0x03) str_cpy(mirror_list[mirror_count].name, "USB HUB");
                    ...
                    /// --- ERGEBNISSE ZEICHNEN ---
                    _15(mirror_scanned) {
                        Text(wx+20, wy+100, "B:D:F", 0xAAAAAA, _128);
                        Text(wx+80, wy+100, "VENDOR DEVICE", 0xAAAAAA, _128);
                        Text(wx+190, wy+100, "BASE ADDR", 0xAAAAAA, _128);
                        Text(wx+280, wy+100, "REFLECTION", 0xAAAAAA, _128);
                        Text(wx+380, wy+100, "CLASS", 0xAAAAAA, _128);
                        _43 ry = wy+120;
                        _39(_43 i=0; i<mirror_count; i++) {
                            _30 s_b[10], s_v[10], s_d[10], s_bar[20], s_ref[20];
                            int_to_str(mirror_list[i].bus, s_b);
                            hex_to_str(mirror_list[i].vendor, s_v);
                            hex_to_str(mirror_list[i].device, s_d);
                            /// 32-Bit Hex in zwei 16-Bit Hälften aufteilen (für dein hex_to_str)
                            _30 h1[10], h2[10];
                            hex_to_str((_182)(mirror_list[i].bar0 >> 16), h1); hex_to_str((_182)(mirror_list[i].bar0 & 0xFFFF), h2);
                            str_cpy(s_bar, h1); str_cpy(s_bar+4, h2);
                            hex_to_str((_182)(mirror_list[i].reflection >> 16), h1); hex_to_str((_182)(mirror_list[i].reflection & 0xFFFF), h2);
                            str_cpy(s_ref, h1); str_cpy(s_ref+4, h2);
                            Text(wx+20, ry, s_b, 0x888888, _86);
                            Text(wx+80, ry, s_v, 0x00FF00, _86); Text(wx+125, ry, s_d, 0x00FF00, _86);
                            Text(wx+190, ry, s_bar, 0xFFFFFF, _86);
                            /// Wenn die Hardware nichts reflektiert (00000000), blockiert sie uns!
                            _89 text_col_ref = (mirror_list[i].reflection EQ 0) ? 0xFF0000 : 0xFF00FF;
                            Text(wx+280, ry, s_ref, text_col_ref, _86);
                            Text(wx+380, ry, mirror_list[i].name, 0xCCCCCC, _128);
                            ry += 15;
                        }
                    }
                }
                    ...
                    _15(dsk_view EQ 0) {
                        ...
                        _15(active_drive_idx NEQ -1) {
                            ...
                            _15(input_cooldown EQ 0 AND mouse_just_pressed AND !blocked AND is_over_rect(mouse_x, mouse_y, wx+20, btn_y, 100, 25)) { 
                                _15(is_cd) {
                                    on_boot_button_click(drives[active_drive_idx].base_port);
                                } _41 {
                            ...
                            _15(is_safe) {
                                ...
                                /// ==================================================
								/// BARE METAL FIX: FORMAT CFS & WRITE TO DISK
								/// Passe die X-Koordinate (wx+130) an deinen roten Button an
								/// ==================================================
								_15(input_cooldown EQ 0 AND mouse_just_pressed AND !blocked AND is_over_rect(mouse_x, mouse_y, wx+130, btn_y, 100, 25)) {
									fs_format_drive(); /// <-- Das ist alles! Der Motor übernimmt.
									is_ntfs = 0; 
									input_cooldown = 10;
								}
                            } _41 {
                        ...
                        _39(_43 f=0; f<8; f++) {
                            ...
                            _15(show_file) {
                                ...
                                /// --- BARE METAL Klick-Logik DSK_VIEW 1 ---
                                _15(input_cooldown EQ 0 AND mouse_just_pressed AND !blocked AND is_over_rect(mouse_x, mouse_y, wx+20, y_off, ww-40, 20)) { 
                                    ...
                                    _15(file_table[f].is_folder) { 
                                        ...
                                        note_r=0; note_c=0;
                                        /// --- BARE METAL FIX: TEXT LOAD (64 MB ALIGNMENT) ---
                                        ...
                                        _15(start_lba NEQ 0) {
                                            ...
                                            /// Den Text zeilenweise ins Notepad kopieren
                                            _43 idx = 0; 
                                            _39(_43 r=0; r<10; r++) { 
                                                _43 c = 0;
                                                _114(c < 39) { 
                ...
                _15(win->id EQ 2) {
                    _15(save_step EQ 0) { 
                        ...
                        _15(mouse_just_pressed AND !blocked AND sel AND drive_count > 0) {
                            active_drive_idx++;
                            _15(active_drive_idx >= drive_count) active_drive_idx = 0;
                            ahci_read_sectors(drives[active_drive_idx].base_port, 1002, 1, (_89)file_table);
                            current_path_id = 255; 
                            input_cooldown = 10;
                        }
                        ...
                        _15(input_cooldown EQ 0 AND mouse_just_pressed AND is_over_rect(mouse_x, mouse_y, wx+ww-100, wy+wh-40, 80, 25)) { 
                            fs_save_file(save_filename, 512);
                            win->open = _86; 
                            input_cooldown = 10; 
                        }
                        ...
                        _15(input_cooldown EQ 0 AND mouse_just_pressed AND is_over_rect(mouse_x, mouse_y, wx+ww-100, wy+wh-40, 80, 25)) { 
                            fs_create_folder(new_folder_name); 
                            save_step = 0; input_cooldown = 10; 
                        }
                    }
                }
                _15(win->id EQ 5) {
                    _15(key_new AND win_z[12] EQ 5) {
                        _15(last_key EQ 0x1C) { 
							...
							_41 _15(str_starts(cmd_input, "SCAN")) {
									str_cpy(cmd_status, "STARTING HARDWARE SCAN...");
									pci_scan_all();
									_15(nic_count > 0) {
										/// BARE METAL FIX: Direkter Beweis, dass die MAC da ist!
										str_cpy(cmd_status, "MAC LOADED: ");
										_30* s = cmd_status + 12;
										_30* m = mac_str;
										_114(*m) *s++ = *m++;
										*s = 0;
									} _41 {
										str_cpy(cmd_status, "ERR: PCI BUS EMPTY (0 NICS)");
									}
							...
							_15(model[0] EQ 0) {
								str_cat(cmd_last_out, "LEER (DMA FEHLGESCHLAGEN)");
							} _41 {
...
_50 execute_kernel() {
    ...
    /// Paket schnüren
    _72 BootInfo info; 
    info.framebuffer_addr = main_gfx_card.framebuffer_addr;
    info.screen_width = 1920;  
    info.screen_height = 1080;
    info.ahci_drive_count = drive_count;
    info.drives_list_ptr = (_89)&drives[0];
    str_cpy(cmd_status, "HANDOFF TO KERNEL.BIN...");
    /// Ziel anvisieren und springen
    kernel_entry_t kernel_main = (kernel_entry_t)0x1000000;
    _33 _192("cli"); 
    kernel_main(&info); 
}